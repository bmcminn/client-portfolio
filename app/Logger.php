...
class Logger {
     ...
     * @param string    $logger['dir']        defaults '.';     defines where log files should be written to
    ...
    public static function config($name='log', $opts=[]) {
        ...
        $opts['name']       = $name;
        $opts['filepath']   = $opts['dir'] . '/' . date($opts['dateFormat']) . '-' . $opts['name'] . '.log';
        $opts['level']      = self::$levels[$opts['level']][0];
